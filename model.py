...
class VistaNet:
  ...
  def __init__(self, hidden_dim, att_dim, emb_size, num_images, num_classes, use_lstm=False):
  ...
  def _init_word_encoder(self):
    with tf.variable_scope('word') as scope:
      ...
      if self.use_lstm:
        cell_fw = rnn.LSTMBlockCell(self.hidden_dim)
        cell_bw = rnn.LSTMBlockCell(self.hidden_dim)
        
        init_state_fw = cell_fw.zero_state(
            batch_size=get_shape(word_rnn_inputs)[0],
            dtype=tf.float32,
        )
        init_state_bw = cell_bw.zero_state(
            batch_size=get_shape(word_rnn_inputs)[0],
            dtype=tf.float32,
        )
  ...
  def _init_sent_encoder(self):
    with tf.variable_scope('sentence') as scope:
      ...
      if self.use_lstm:
        ...
        init_state_fw = cell_fw.zero_state(
            batch_size=get_shape(sentence_rnn_inputs)[0],
            dtype=tf.float32,
        )
        init_state_bw = cell_fw.zero_state(
            batch_size=get_shape(sentence_rnn_inputs)[0],
            dtype=tf.float32,
        )
      ...
      if self.use_lstm:
        self.sentence_outputs, _ = text_attention(inputs=sentence_rnn_outputs,
                                att_dim=self.att_dim,
